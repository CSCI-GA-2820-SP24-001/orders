...
class TestRoutesService(TestCase):
    ...
    def test_query_by_customer_id(self):
        ...
        self.assertEqual(resp.status_code, status.HTTP_200_OK)
        self.assertEqual(len(resp.json), 3)
        resp = self.client.get("/orders?customer_id=1")
        self.assertEqual(resp.status_code, status.HTTP_200_OK)
        self.assertEqual(len(resp.json), 1)
    ...
    def test_query_by_order_date(self):
        ...
        # Create some orders
        self.client.post("/orders", json={"customer_id": 1, "order_date": "2021-01-01"})
        self.client.post("/orders", json={"customer_id": 2})
        self.client.post("/orders", json={"customer_id": 3, "order_date": "2021-01-01"})
        resp = self.client.get("/orders")
        self.assertEqual(resp.status_code, status.HTTP_200_OK)
        self.assertEqual(len(resp.json), 3)
        resp = self.client.get("/orders?order_date=2021-01-01")
        self.assertEqual(resp.status_code, status.HTTP_200_OK)
        self.assertEqual(len(resp.json), 2)
    ...
    def test_query_by_status(self):
        ...
        # Create some orders
        self.client.post("/orders", json={"customer_id": 1, "status": "pending"})
        self.client.post("/orders", json={"customer_id": 2, "status": "processing"})
        self.client.post("/orders", json={"customer_id": 3, "status": "shipped"})
        self.client.post("/orders", json={"customer_id": 4, "status": "delivered"})
        self.client.post("/orders", json={"customer_id": 5, "status": "cancelled"})
        self.client.post("/orders", json={"customer_id": 6, "status": "delivered"})
        self.client.post("/orders", json={"customer_id": 7, "status": "delivered"})
        resp = self.client.get("/orders")
        self.assertEqual(resp.status_code, status.HTTP_200_OK)
        ...
        self.assertEqual(resp.status_code, status.HTTP_200_OK)
        self.assertEqual(len(resp.json), 1)
        resp = self.client.get("/orders?status=delivered")
        self.assertEqual(resp.status_code, status.HTTP_200_OK)
        self.assertEqual(len(resp.json), 3)

    def test_query_by_tracking_number(self):
        """test_query_by_tracking_number"""
        # Create some orders
        self.client.post("/orders", json={"customer_id": 1, "tracking_number": "aaaaaaa"})
        self.client.post("/orders", json={"customer_id": 2, "tracking_number": "bbbbbbb"})
        self.client.post("/orders", json={"customer_id": 3, "tracking_number": "aaaaaaa"})
        resp = self.client.get("/orders")
        self.assertEqual(resp.status_code, status.HTTP_200_OK)
        self.assertEqual(len(resp.json), 3)
        resp = self.client.get("/orders?tracking_number=aaaaaaa")
        self.assertEqual(resp.status_code, status.HTTP_200_OK)
        self.assertEqual(len(resp.json), 2)
    ...
    def test_query_by_discount_amount(self):
        ...
        # Create some orders
        self.client.post("/orders", json={"customer_id": 1, "discount_amount": 0.0})
        self.client.post("/orders", json={"customer_id": 2, "discount_amount": 10.0})
        self.client.post("/orders", json={"customer_id": 3, "discount_amount": 0.0})
        resp = self.client.get("/orders")
        self.assertEqual(resp.status_code, status.HTTP_200_OK)
        self.assertEqual(len(resp.json), 3)
        resp = self.client.get("/orders?discount_amount=0.0")
        self.assertEqual(resp.status_code, status.HTTP_200_OK)
        self.assertEqual(len(resp.json), 2)
